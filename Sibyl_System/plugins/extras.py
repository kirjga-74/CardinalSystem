...
async def addsbm(event) -> None:
    ...
    if u_id in MANAGERS:
        await System.send_message(event.chat_id, "That person is already spam block manager!")
        return
    if HEROKU:
        ...
        await System.send_message(event.chat_id, "Added to spam block manager, Restarting...")
...
@System.on(system_cmd(pattern=r"spamblockmanager", allow_developers=True))
